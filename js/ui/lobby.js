  ...
  if (!character) {
    return `
      <div class="pixel-card">
        ...
        <p class="mt-2 text-sm text-amber-300">Create one before entering any room.</p>
      </div>
    `;
  }
    ...
    <div class="pixel-card">
      <div class="flex items-start justify-between gap-3">
        <div>
          ...
          <p class="mt-2 text-xl font-semibold text-slate-100">${character.name}</p>
...
export function createLobbyScreen(app) {
  ...
  return {
    ...
    render() {
      section.innerHTML = `
        <div>
          <p class="pixel-title text-lg text-accent">Lobby</p>
          <p class="mt-4 text-sm leading-7 text-slate-300">
            Every session starts here. Build a character, configure controls, then host or join a local room.
          </p>
        </div>
